...
namespace Robust.Client.UserInterface.Controls
{
    ...
    /// </summary>
    [Virtual]
    public class LayeredColorTextureRect : Control
    {
        public const string StylePropertyShader = "shader";

        private bool _canShrink;
        private List<(Texture, Color?)> _textures = new();
        ...
        /// </summary>
        public List<(Texture, Color?)> Textures
        {
            get => _textures;
            set
            {
            ...
            foreach (var texture in Textures)
            {
                ...
                switch (Stretch)
                {
                    case TextureRect.StretchMode.Scale:
                        handle.DrawTextureRect(texture.Item1,
                            UIBox2.FromDimensions(Vector2.Zero, PixelSize), texture.Item2);
                        break;
                    case TextureRect.StretchMode.Tile:
                    // TODO: Implement Tile.
                    case TextureRect.StretchMode.Keep:
                        handle.DrawTextureRect(texture.Item1,
                            UIBox2.FromDimensions(Vector2.Zero, texture.Item1.Size * _textureScale * UIScale), texture.Item2);
                        break;
                    case TextureRect.StretchMode.KeepCentered:
                    {
                        var position = (PixelSize - texture.Item1.Size * _textureScale * UIScale) / 2;
                        handle.DrawTextureRect(texture.Item1, UIBox2.FromDimensions(position, texture.Item1.Size * _textureScale * UIScale), texture.Item2);
                        break;
                    }
                    ...
                    {
                        var (texWidth, texHeight) = texture.Item1.Size * _textureScale;
                        ...
                        handle.DrawTextureRectRegion(texture.Item1, UIBox2.FromDimensions(position, size), null, texture.Item2);
                        break;
                    }

                    case TextureRect.StretchMode.KeepAspectCovered:
                        var texSize = texture.Item1.Size * _textureScale;
                        ...
                        handle.DrawTextureRectRegion(texture.Item1, PixelSizeBox, UIBox2.FromDimensions(offset, PixelSize / scale), texture.Item2);
                        break;
                    default:
        ...
        protected override Vector2 MeasureOverride(Vector2 availableSize)
        {
            ...
            foreach (var texture in Textures)
            {
                largestX = Math.Max(texture.Item1.Width, largestX);
                largestY = Math.Max(texture.Item1.Height, largestY);
            }
